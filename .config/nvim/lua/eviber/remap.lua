...
-- on save, copy file to same directory in ~/Document/FADEX/<dir>, if applicable
local function copy_file_to_parent_parent_directory()
	...
	local dest_dir = 'C:/Users/FT999180/Documents/FADEX/.nvim_rulechecker/headers'
	local filename = vim.fn.fnamemodify(filepath, ":p:t") -- Get the file name

	-- Check if the parent of the parent directory exists
	if vim.fn.isdirectory(dest_dir) == 1 then
		local dest = dest_dir .. '/' .. filename
		os.execute('cp ' .. filepath .. ' ' .. dest) -- Copy the file
	end
end

vim.api.nvim_create_autocmd('BufWritePost', {
	pattern = {'*.h'},
	callback = copy_file_to_parent_parent_directory,
})
...
vim.keymap.set('n', '<leader>sc', function() vim.opt.scroll = 1; end, { desc = "set scroll=1" })
